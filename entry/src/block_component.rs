...
impl BlockComponent {
    ...
    /// Parses a single BlockComponent from the data and returns it along with the number of bytes consumed.
    fn parse_single_from_bytes(data: &[u8]) -> Result<(Self, usize), BlockComponentError> {
        ...
        // Check what's after the entries
        ...
        match (entries.is_empty(), remaining_bytes.is_empty()) {
            (true, true) => {
                // Empty entries, no marker
                Ok((Self::Entries(Vec::new()), cursor))
            }
            (true, false) => {
                // No entries, but there's marker data
                // Determine marker size and parse it
                let marker_size = Self::get_versioned_marker_size(remaining_bytes)?;
            ...
            (false, true) => {
                // Non-empty entries, no additional data
                Ok((Self::Entries(entries), cursor))
            }
            (false, false) => {
                // Non-empty entries with additional data
                // In the multi-component case, this is fine - the additional data is the next component
                // Just return the entries component
                Ok((Self::Entries(entries), cursor))
            }
        }
    }
    ...
    fn get_versioned_marker_size(data: &[u8]) -> Result<usize, BlockComponentError> {
        ...
        // For both V1 and V2 markers, the format is:
        ...
        D: Deserializer<'de>,
    {
        ...
        impl Visitor<'_> for BlockComponentVisitor {
            ...
            where
                E: de::Error,
            {
                // Parse all components and return the first one for backward compatibility
                // If there are multiple components, this will only deserialize the first
                let components = BlockComponent::from_bytes(value).map_err(de::Error::custom)?;
                components
                    .into_iter()
                    .next()
                    .ok_or_else(|| de::Error::custom("No BlockComponent found in data"))
            }
        }
...
mod tests {
    ...
    fn test_block_component_invalid_mixed_data() {
        ...
        // Create a proper BlockComponent with the marker
        let marker_component = BlockComponent::BlockMarker(marker);
        ...
        // Should parse as two components now
    ...
    fn test_block_component_empty_default_serialization() {
        ...
        assert_eq!(deserialized.len(), 1);
        assert_eq!(component, deserialized[0]);
        assert!(deserialized[0].is_entries());
        assert_eq!(deserialized[0].entries().len(), 0);
    }
    ...
    fn test_block_component_malformed_data() {
        // Empty data should return empty Vec
        assert!(BlockComponent::from_bytes(&[]).is_ok());
        assert_eq!(BlockComponent::from_bytes(&[]).unwrap().len(), 0);
    ...
    fn test_block_component_large_entries() {
        ...
        assert_eq!(deserialized.len(), 1);
        assert_eq!(component, deserialized[0]);
        assert_eq!(deserialized[0].entries().len(), 1000);
    }
    ...
    fn test_block_component_valid_entries_only() {
        ...
        let deserialized = BlockComponent::from_bytes(&bytes).unwrap();
        assert_eq!(deserialized.len(), 1);
        assert_eq!(deserialized[0].entries().len(), 3);
    ...
    fn test_block_component_multiple_components() {
        ...
        let component1 = BlockComponent::new_entries(entries1).unwrap();
        ...
        let component3 = BlockComponent::new_entries(entries3).unwrap();
        ...
        assert!(deserialized[0].is_entries());
        assert_eq!(deserialized[0].entries().len(), 2);
        ...
        assert!(deserialized[2].is_entries());
        assert_eq!(deserialized[2].entries().len(), 1);
    ...
    fn test_block_component_with_mixed_entry_sizes() {
        ...
        let deserialized = BlockComponent::from_bytes(&bytes).unwrap();
        assert_eq!(deserialized.len(), 1);
        assert_eq!(deserialized[0].entries().len(), 10);
        assert!(deserialized[0].marker().is_none());
    }
    ...
    fn test_multiple_entry_components() {
        // Test deserializing multiple BlockComponents with Vec<Entry>

        // Create multiple entry components
        let entries1 = vec![Entry::default()];
        let component1 = BlockComponent::new_entries(entries1).unwrap();

        let entries2 = vec![Entry::default(), Entry::default(), Entry::default()];
        let component2 = BlockComponent::new_entries(entries2).unwrap();
        ...
        assert!(deserialized[0].is_entries());
        assert_eq!(deserialized[0].entries().len(), 1);

        assert!(deserialized[1].is_entries());
        assert_eq!(deserialized[1].entries().len(), 3);

        assert!(deserialized[2].is_entries());
        assert_eq!(deserialized[2].entries().len(), 2);
    }

    #[test]
    fn test_mixed_entries_and_markers() {
        // Test with entries, ParentUpdate, BlockFooter combinations

        // Create entry component
        let entries = vec![Entry::default(), Entry::default()];
        let entry_component = BlockComponent::new_entries(entries).unwrap();

        // Create ParentUpdate marker component
        let parent_update = UpdateParentV1 {
            new_parent_slot: 12345,
            new_parent_block_id: Hash::new_unique(),
        };
        let parent_marker = VersionedBlockMarker::V2(BlockMarkerV2::UpdateParent(
        ...
        let entry_component2 = BlockComponent::new_entries(entries2).unwrap();
        ...
        // Verify second component (ParentUpdate marker)
        assert!(deserialized[1].is_marker());
        let marker1 = deserialized[1].marker().unwrap();
        assert_eq!(marker1.version(), 2);
        if let VersionedBlockMarker::V2(BlockMarkerV2::UpdateParent(update)) = marker1 {
            ...
            panic!("Expected V2 UpdateParent");
        }
    ...
    fn test_all_marker_types_sequence() {
        ...
        // Create another BlockFooter with different data
        let footer2 = BlockFooterV1 {
            block_producer_time_nanos: 33333,
            block_user_agent: b"node-2".to_vec(),
        };
        let footer_marker2 = VersionedBlockMarker::new(BlockMarkerV1::BlockFooter(
            VersionedBlockFooter::new(footer2),
        ));
        let footer_component2 = BlockComponent::new_block_marker(footer_marker2);

        // Serialize all
        let mut bytes = Vec::new();
        bytes.extend_from_slice(&footer_component.to_bytes().unwrap());
        bytes.extend_from_slice(&parent_component.to_bytes().unwrap());
        bytes.extend_from_slice(&footer_component2.to_bytes().unwrap());
    ...
    fn test_large_sequence_of_components() {
        ...
        for i in 0..num_pairs {
            ...
            assert!(deserialized[entry_idx].is_entries());
            assert_eq!(deserialized[entry_idx].entries().len(), i + 1);
    ...
    #[test]
    fn test_empty_entries_between_markers() {
        // Test with empty entry components between markers

        let empty_component = BlockComponent::default(); // Empty entries
        ...
        // Serialize: empty entries, footer, empty entries, footer
        let mut bytes = Vec::new();
        bytes.extend_from_slice(&empty_component.to_bytes().unwrap());
        bytes.extend_from_slice(&footer_component.to_bytes().unwrap());
        bytes.extend_from_slice(&empty_component.to_bytes().unwrap());
        bytes.extend_from_slice(&footer_component.to_bytes().unwrap());
        ...
        assert!(deserialized[2].is_entries());
        assert_eq!(deserialized[2].entries().len(), 0);
    ...
    fn test_realistic_block_sequence() {
        ...
        // Verify entries
        assert_eq!(deserialized[0].entries().len(), 5);
        assert_eq!(deserialized[1].entries().len(), 3);
        assert_eq!(deserialized[2].entries().len(), 7);
