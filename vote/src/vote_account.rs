...
mod tests {
    ...
    fn new_rand_vote_account<R: Rng>(
        ...
        AccountSharedData::new_data(
            lamport.unwrap_or_else(|| rng.gen()), // lamports
            &vote_state_versions,
            &solana_sdk_ids::vote::id(), // owner
        )
        .unwrap()
    ...
    fn new_rand_vote_accounts<R: Rng>(
        ...
        repeat_with(move || {
            ...
            let account = new_rand_vote_account(rng, Some(node), is_alpenglow, None);
    ...
    fn test_vote_account_try_from(is_alpenglow: bool) {
        let mut rng = rand::thread_rng();
        let account = new_rand_vote_account(&mut rng, None, is_alpenglow, None);
    ...
    fn test_vote_account_try_from_invalid_owner(is_alpenglow: bool) {
        let mut rng = rand::thread_rng();
        let mut account = new_rand_vote_account(&mut rng, None, is_alpenglow, None);
    ...
    fn test_vote_accounts_deserialize_invalid_account(is_alpenglow: bool) {
        ...
        let valid_account = new_rand_vote_account(&mut rng, None, is_alpenglow, None);
    ...
    fn test_staked_nodes_update(is_alpenglow: bool) {
        ...
        let pubkey = Pubkey::new_unique();
        let node_pubkey = Pubkey::new_unique();
        let account1 = new_rand_vote_account(&mut rng, Some(node_pubkey), is_alpenglow, None);
        ...
        let account2 = new_rand_vote_account(&mut rng, Some(node_pubkey), is_alpenglow, None);
        ...
        let account3 = new_rand_vote_account(&mut rng, Some(new_node_pubkey), is_alpenglow, None);
    ...
    fn test_staked_nodes_zero_stake(is_alpenglow: bool) {
        ...
        let new_node_pubkey = Pubkey::new_unique();
        let account2 = new_rand_vote_account(&mut rng, Some(new_node_pubkey), is_alpenglow, None);
        let vote_account2 = VoteAccount::try_from(account2).unwrap();
        let ret = vote_accounts.insert(pubkey, vote_account2.clone(), || {
            panic!("should not be called")
        });
    ...
    fn test_clone_and_filter_for_alpenglow_truncates() {
        ...
        let filtered =
            vote_accounts.clone_and_filter_for_alpenglow(lower_limit, MIN_STAKE_FOR_STAKED_ACCOUNT);
    ...
    fn test_clone_and_filter_for_alpenglow_filters_non_alpenglow() {
        ...
        let filtered =
            vote_accounts.clone_and_filter_for_alpenglow(new_limit, MIN_STAKE_FOR_STAKED_ACCOUNT);
    ...
    fn test_clone_and_filter_for_alpenglow_same_stake_at_border() {
        ...
        let accounts = (0..num_accounts).map(|index| {
            let account = new_rand_vote_account(&mut rng, None, true, Some(10_000_000_000));
            let vote_account = VoteAccount::try_from(account).unwrap();
        ...
        let filtered = vote_accounts
            .clone_and_filter_for_alpenglow(num_accounts, MIN_STAKE_FOR_STAKED_ACCOUNT);
        assert_eq!(filtered.len(), num_accounts);
        let filtered = vote_accounts
            .clone_and_filter_for_alpenglow(num_alpenglow_nodes, MIN_STAKE_FOR_STAKED_ACCOUNT);
        assert_eq!(filtered.len(), num_alpenglow_nodes - 10);
    }

    #[test]
    fn test_clone_and_filter_for_alpenglow_not_enough_lamports() {
        let mut rng = rand::thread_rng();
        let num_alpenglow_nodes = 2000;
        ...
        let filtered = vote_accounts
            .clone_and_filter_for_alpenglow(num_alpenglow_nodes, minimum_vote_account_balance);
